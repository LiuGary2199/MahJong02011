...
public class CashOutPanel : RealUITruth
{
    ...
    private void Start()
    {
        CashOutManager.HowWhatever()._CashOutPanel = this;
        CloseBtn.onClick.AddListener(() => { DodgeUIEddy(nameof(CashOutPanel)); });
        CashOutBtn.onClick.AddListener(() => { OnCashOutBtn(); });
        MoneyName = CryBustPeg.instance.FareIts_Soul.MoneyName;
        InfoText.text = CryBustPeg.instance.FareIts_Soul.Description;
        InfoPanel_OpenBtn.onClick.AddListener(() => OpenPanel(InfoPanel));
        InfoPanel_CloseBtn.onClick.AddListener(() => ClosePanel(InfoPanel));
        ContactBtn.GetComponent<Text>().text = CryBustPeg.instance.RealFee;
        ContactBtn.onClick.AddListener(() =>
        {
            string url = CryBustPeg.instance.RealFee;
        ...
        RecordPanel_OpenBtn.onClick.AddListener(() =>
        {
            ...
            CashOutManager.HowWhatever().GetWithdrawRecord();
        });
        ...
        AccountText.text = CashOutManager.HowWhatever().Account;
        Confirm_AccountText.text = CashOutManager.HowWhatever().Account;
        if (string.IsNullOrEmpty(AccountText.text))
        {
            AccountText.text = "Account";
            Confirm_AccountText.text = "Account";
        }
        IDText.text = "UID:" + CashOutManager.HowWhatever().Data.UserID;
        PolicyBtn.onClick.AddListener(() =>
        {
            string url = CryBustPeg.instance.RealFee + "/privacy_policy.html";
#if UNITY_ANDROID || UNITY_EDITOR
            Application.OpenURL(url);
#elif UNITY_IOS
        openUrl(url);
        ...
        PolicyToggle.isOn = BondSoulEvening.HowTape("CashOut_PolicyAgree");
    ...
    {
        base.Hidding();
        CashOutManager.HowWhatever().WaitToSendEvent1304();
        LullGuinea.FareItsDwarfInsectEndear?.Invoke();
    }

    public void UpdateUserInfo() //拉取用户信息 显示加载界面
    {
        //StartLoadingAnim(UpdateUI_Loading);
        CashOutManager.HowWhatever().UpdateUserInfo();
    }
    ...
    {
        if (CashOutManager.HowWhatever().Data == null)
            return;
        if (string.IsNullOrEmpty(CashOutManager.HowWhatever().Account))
        {
            OpenPanel(AccountPanel);
            return;
        }
    ...
    void OnConfimBtn() //确认修改账户
    {
        ...
        {
            AllayEvening.HowWhatever().KnotAllay("Account cannot be empty");
            return;
        }
        if (!string.IsNullOrEmpty(AccountErrorText.text))
        {
            AllayEvening.HowWhatever().KnotAllay(AccountErrorText.text);
            return;
        }

        CashOutManager.HowWhatever().Account = AccountInput.text;
        BondSoulEvening.OldCoyote("CashOut_Account", AccountInput.text);
    ...
    void OnSbmitBtn() //确认是否同意协议 同意后调用提现接口
    {
        ...
        {
            AllayEvening.HowWhatever().KnotAllay("Please agree to the policy");
            return;
        }
        BondSoulEvening.OldTape("CashOut_PolicyAgree", true);
        StartLoadingAnim(CashOutBtn_Loading);
        StartLoadingAnim(SbmitBtn_Loading);
        CashOutManager.HowWhatever().Withdraw();
    }
    ...
    public void UpdateMoney()
    {
        ...
        float MoneyEnd = CashOutManager.HowWhatever().Money;
        DOTween.To(() => MoneyStart, x => MoneyText.text = x.ToString("F2"), MoneyEnd, 1f);
        CashText.text = CashOutManager.HowWhatever().Data.Cash.ToString("F2");
        //进度条动画
        float MaxMoney = float.Parse(CryBustPeg.instance.FareIts_Soul.convert_goal, CultureInfo.CurrentCulture);
    ...
    public void MoneyToCashAnim()
    {
        ...
        float CashOutEnd = CashOutManager.HowWhatever().Data.Cash;
    ...
    {
        if (CashOutManager.HowWhatever().Data.TaskData != null && CashOutManager.HowWhatever().Data.TaskData.Name != "Null")
        {
            // Cash余额任务 不走前端进度统计 使用后台Data.Cash数值
            if (CashOutManager.HowWhatever().Data.TaskData.Name == "Cash" && CashOutManager.HowWhatever().Data.Cash < CashOutManager.HowWhatever().Data.TaskData.Target)
            {
                CashOutBtn.gameObject.SetActive(false);
                CashOutBtn_Task.SetActive(true);
                CashOutBtn_Task_Text.text = string.Format(CashOutManager.HowWhatever().Data.TaskData.Description, CashOutManager.HowWhatever().Data.Cash, CashOutManager.HowWhatever().Data.TaskData.Target);
            }
            // 其他任务 前端统计任务完成进度 
            else if (CashOutManager.HowWhatever().Data.TaskData.Name != "Cash" && CashOutManager.HowWhatever().Data.TaskData.NowValue < CashOutManager.HowWhatever().Data.TaskData.Target)
            {
                CashOutBtn.gameObject.SetActive(false);
                CashOutBtn_Task.SetActive(true);
                CashOutBtn_Task_Text.text = string.Format(CashOutManager.HowWhatever().Data.TaskData.Description, CashOutManager.HowWhatever().Data.TaskData.NowValue, CashOutManager.HowWhatever().Data.TaskData.Target);
            }
            // 任务完成 显示提现按钮
            else
            {
    ...
    public void UpdateRecord(int Page = 0) //更新单页10个提现记录
    {
        ...
        if (CashOutManager.HowWhatever().Data.Record == null || CashOutManager.HowWhatever().Data.Record.Count == 0)
        {
            NoRecordTip.SetActive(true);
            return;
        }

        int RecordCount = CashOutManager.HowWhatever().Data.Record.Count;
        ...
        List<WithdrawRecordItem> OnePageRecord = CashOutManager.HowWhatever().Data.Record.GetRange(startIndex, itemsToShow);
    ...
    void ShowGuide()
    {
        ...
        string[] Descriptions = new string[]
        {
         ...
         for (int i = 0; i < GuideItems1.Length; i++)
        {
            GuideItems1[i].DOKill(true);
            GuideItems2[i].DOKill(true);
        }
        if (GuideStep == 0)
        {
        ...
        GuideTitleText.text = "";
        int TempGuideStep = GuideStep;
        GuideTitleText.DOFade(0, .5f).OnComplete(() => GuideTitleText.text = Titles[TempGuideStep]);
        GuideTitleText.DOFade(1, .5f).SetDelay(.51f);
        GuideText.DOKill();
        GuideText.text = "";
        GuideText.DOText(Descriptions[TempGuideStep], 1f);
        for (int i = 0; i < GuideStepPoints.Length; i++)
            GuideStepPoints[i].SetActive(i == TempGuideStep);
    }
    ...
    public void CloseLoading_Withdraw(bool IsCashOutSuccess = false)
    {
        ...
        if (IsCashOutSuccess)
        {
            AllayEvening.HowWhatever().KnotAllay("Withdraw success");
            ClosePanel(ConfirmAccountPanel);
        }
    }
    ...
    {
        SpitAnvilPawnee.HowWhatever().HeroAnvil("1301", LeftTimeText.text, CashOutManager.HowWhatever().Money.ToString());
    }
}
