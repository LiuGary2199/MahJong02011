...
public class CashOutManager : AfarDepiction<CashOutManager>
{
    [Header("登录平台")]
    ...
    public string RealFee = "http://us.kittenwhiskers.top";
    ...
    private void Start()
    {
        Account = BondSoulEvening.HowCoyote("CashOut_Account");
        Money = BondSoulEvening.HowHairy("CashOut_Money");
    }
    ...
    private void OnApplicationPause(bool pauseStatus)
    {
        ...
        if (pauseStatus && Seconds > 0)
        {
            ...
            string info = $"All {CryBustPeg.instance.FareIts_Soul.MoneyName} have been converted,Please check your rewards!";
            ConservativeEvening.Whatever.CedarConservative();
            ConservativeEvening.Whatever.UnwieldyConservative(title, info, (int)Seconds);
            for (int i = 0; i < 10; i++) // 10次延时 10800秒 3小时
                ConservativeEvening.Whatever.UnwieldyConservative(title, info, (int)Seconds + (i * 10800));
        }
    ...
    void TimeCount() //计时
    {
        if (Data != null)
        {
            ...
            //任务
            if (CryBustPeg.instance.FareIts_Soul != null && CryBustPeg.instance.FareIts_Soul.TaskList.Count > 0)
            {
                //记录第一次登录日期utc
                ...
                if (Day >= CryBustPeg.instance.FareIts_Soul.TaskList.Count) //一天一任务 天数超出任务数量显示默认任务
                    Data.TaskData = CryBustPeg.instance.FareIts_Soul.TaskList.FirstOrDefault(t => t.IsDefault);
                else
                    Data.TaskData = CryBustPeg.instance.FareIts_Soul.TaskList[Day];
    ...
    {
        Money += Value;
        BondSoulEvening.OldHairy("CashOut_Money", Money);
    ...
    public void SendEvent1304() ////打点 关闭商店后行为
    {
        ...
        if (Event_1304Time <= 0)
            return;
        SpitAnvilPawnee.HowWhatever().HeroAnvil("1304", Event_1304Time.ToString());
        Event_1304Time = 0;
    }
    ...
    Dictionary<string, string> Headers() // 请求头
    {
        ...
        return new Dictionary<string, string>
        {
            ...
            {"Authorization", BondSoulEvening.HowCoyote("CashOut_Token")},
    ...
    public void Login() // 登录
    {
        ...
        {
            Platform = "Android";
            DeviceAdId = BondSoulEvening.HowCoyote("gaid");
            if (Application.platform == RuntimePlatform.Android)
            {
        ...
        else
        {
            Platform = "iOS";
            Manufacturer = "Apple";
            DeviceAdId = BondSoulEvening.HowCoyote("idfv");
        }
        ...
        bool hasuuidAndAppid = BondSoulEvening.HowTape("UuidAndAPPid");
        ...
        string loginUrl = $"{RealFee}/login";
        CashOutLog($"请求登录  请求体: {jsonBody}", false);

        CryCyanEvening.HowWhatever().PileSpitMode(
            url: loginUrl,
            jsonData: jsonBody,
            success: (result) =>
            {
                try
                {
                    ...
                    if (response.code == 0)
                    {
                        ...
                        bool isNewPlayer = !PlayerPrefs.HasKey(CShaman.It_IDNssUntold + "Bool") || BondSoulEvening.HowTape(CShaman.It_IDNssUntold);
                        if (isNewPlayer)
                            BondSoulEvening.OldTape("UuidAndAPPid", true);
                        //刷新token 获取提现规则
                        BondSoulEvening.OldCoyote("CashOut_Token", response.data.token);
                        GetWithdrawRule();
                        //整理数据
                        ...
                        if (PlayerPrefs.HasKey("CashOut_ConvertTime"))
                            Data.ConvertTime = long.Parse(BondSoulEvening.HowCoyote("CashOut_ConvertTime"));
                        ...
                        BondSoulEvening.OldCoyote("CashOut_ConvertTime", Data.ConvertTime.ToString());
                    ...
                    {
                        AllayEvening.HowWhatever().KnotAllay("Login fail :" + response.msg);
            ...
            fail: () =>
            {
                ...
                AllayEvening.HowWhatever().KnotAllay("Login fail");
                Ready = false;
            },
            timeout: 3f,
            headers: Headers()
    ...
    public void UpdateUserInfo() // 更新用户信息
    {
        ...
        string url = $"{RealFee}/user";
        CryCyanEvening.HowWhatever().HttpHow(
            url: url,
            success: (result) =>
            {
                try
                {
                    ...
                    if (response.code == 0)
                    {
                        ...
                        double OldCash = BondSoulEvening.HowSyntax("CashOut_Cash");
                        ...
                        if (Money == 0)
                        {
                            ...
                            //打点 如果钱转化了 上报转化信息
                            if (IsIconFly)
                                SpitAnvilPawnee.HowWhatever().HeroAnvil("1302", Event_Money, NewCash.ToString());
                        }
                        else
                        {
                            Data.Cash = NewCash;
                        ...
                        BondSoulEvening.OldSyntax("CashOut_Cash", Data.Cash);
    ...
    {
        string url = $"{RealFee}/withdraw/rule?platform={WithdrawPlatform}";
    ...
    public void Withdraw() // 提现
    {
        ...
        {
            AllayEvening.HowWhatever().KnotAllay($"Minimum withdrawal amount {MinWithdrawCount}");
        ...
        CryCyanEvening.HowWhatever().PileSpitMode(
            ...
            success: (result) =>
            {
                ...
                catch (Exception e)
                {
                    ...
                    _CashOutPanel?.CloseLoading_Withdraw();
                    AllayEvening.HowWhatever().KnotAllay("Withdraw fail :" + e.Message);
    ...
    {
        SpitAnvilPawnee.HowWhatever().HeroAnvil("1303", Event_Cash, IsSuccess ? "1" : "0");
    }
    ...
    {
        string url = $"{RealFee}/user/ad";
        RequestData_ReportAdjustID requestData = new RequestData_ReportAdjustID();
        requestData.id = EffortWineEvening.Instance.HowEffortZinc();
        string jsonBody = JsonMapper.ToJson(requestData);
    ...
    {
        if (string.IsNullOrEmpty(BondSoulEvening.HowCoyote("CashOut_Token")))
        {
    ...
    public void AddTaskValue(string Name, float Value) //增加任务值
    {
        if (Data.TaskData != null && Data.TaskData.Name == Name)
        {
            ...
            {
                SpitAnvilPawnee.HowWhatever().HeroAnvil("1305", Name, NewValue.ToString(), Data.TaskData.IsDefault.ToString());
