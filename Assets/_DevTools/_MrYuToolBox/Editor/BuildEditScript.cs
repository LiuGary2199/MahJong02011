...
public class BuildEditScript : MonoBehaviour
{
    ...
    public static void Check(int buildTargetIndex, BuildInfoData info)
    {
        ...
        if (Resources.FindObjectsOfTypeAll<CryBustPeg>()[0].RealFee != info.BaseUrl)
        {
            isSame = false;
            Debug.Log("域名");
        }
        ...
        if (Resources.FindObjectsOfTypeAll<ADEvening>()[0].MAX_SDK_KEY != info.Applovin_SDK_KEY)
        {
            isSame = false;
            Debug.Log("admanager key");
        }
        if (Resources.FindObjectsOfTypeAll<ADEvening>()[0].MAX_REWARD_ID != info.Applovin_REWARD_ID)
        {
            isSame = false;
            Debug.Log("reward id");
        }
        if (Resources.FindObjectsOfTypeAll<ADEvening>()[0].MAX_INTER_ID != info.Applovin_INTER_ID)
        {
            isSame = false;
            Debug.Log("inter id");
        }
        if (Resources.FindObjectsOfTypeAll<CryBustPeg>()[0].LullFeel != info.GameCode)
        {
            isSame = false;
            Debug.Log("gamecode");
        }
    ...
    public static void SuperBuild(int buildTargetIndex, BuildInfoData info)
    {
        ...
        Resources.FindObjectsOfTypeAll<ADEvening>()[0].MAX_SDK_KEY = info.Applovin_SDK_KEY;
        Resources.FindObjectsOfTypeAll<ADEvening>()[0].MAX_REWARD_ID = info.Applovin_REWARD_ID;
        Resources.FindObjectsOfTypeAll<ADEvening>()[0].MAX_INTER_ID = info.Applovin_INTER_ID;
        ...
        if (!GameObject.Find("MainManager") || !GameObject.Find("MainManager").GetComponent<EffortWineEvening>())
        {
            if (GameObject.Find("MainManager"))
            {
                GameObject.Find("MainManager").AddComponent<EffortWineEvening>();
            }
            else
            {
                GameObject mainObj = new GameObject("MainManager");
                mainObj.AddComponent<EffortWineEvening>();
            }
        }
        GameObject.Find("MainManager").GetComponent<EffortWineEvening>().FourthID = info.Adjust_APP_ID;

        if (!GameObject.Find("MainManager").GetComponent<LastGoEvening>())
        {
            GameObject.Find("MainManager").AddComponent<LastGoEvening>();
        }

#if UNITY_IOS
        GameObject.Find("MainManager").GetComponent<LastGoEvening>().Decor = info.Rate_ID;
#endif
#if UNITY_ANDROID
        GameObject.Find("MainManager").GetComponent<LastGoEvening>().Decor = info.PackageName;
#endif
        ...
        GameObject.Find("MainManager").GetComponent<CashOutManager>().RealFee = modifiedUrl;

        Resources.FindObjectsOfTypeAll<CryBustPeg>()[0].LullFeel = info.GameCode;
        Resources.FindObjectsOfTypeAll<CryBustPeg>()[0].RealFee = info.BaseUrl;
        Resources.FindObjectsOfTypeAll<CryBustPeg>()[0].RealStingFee = info.BaseUrl + CShaman.StingUrl;
        Resources.FindObjectsOfTypeAll<CryBustPeg>()[0].RealShamanFee = info.BaseUrl + CShaman.ShamanFee;
        Resources.FindObjectsOfTypeAll<CryBustPeg>()[0].RealSlitFee = info.BaseUrl + CShaman.SlitFee;
        Resources.FindObjectsOfTypeAll<CryBustPeg>()[0].RealEffortFee = info.BaseUrl + CShaman.EffortFee;
        EditorSceneManager.SaveScene(EditorSceneManager.GetActiveScene());

#if UNITY_IOS
        string url = info.BaseUrl + CShaman.ShamanFee + info.GameCode + "&channel=" + "AppStore" + "&version=" + buildWindow.Version;
#elif UNITY_ANDROID
        string url = info.BaseUrl + CShaman.ShamanFee + info.GameCode + "&channel=" + "GooglePlay" + "&version=" + buildWindow.Version;
#else
        string url = info.BaseUrl + CShaman.ShamanFee + info.GameCode + "&channel=" + "GooglePlay" + "&version=" + buildWindow.Version;
#endif
       ...
       (data) =>
       {
           ...
           //if (rootData.data.apple_pie != "apple")
           {
               ...
               if (GameObject.Find("CryCyanEvening"))
               {
                   DestroyImmediate(GameObject.Find("CryCyanEvening"));
               }
           }
       },
       () =>
