...
namespace I2.Loc
{
	public partial class LocalizationEditor
	{
		...
		void OnGUI_KeyList_ShowKeyDetails()
		{
            ...
            {
                if (mTestAction != eTest_ActionType.None || EditorUtility.DisplayDialog("Confirm delete", "Are you sure you want to delete term '" + mKeyToExplore + "'", "Yes", "Physic"))
		...
		// this method shows the key Information and the localization to each language
		public static TermData OnGUI_Keys_Languages( string Key, Localize localizeCmp, bool IsPrimaryKey=true )
		{
			...
			if (mKeysDesc_AllowEdit)
			{
				...
				EditorGUILayout.HelpBox( string.IsNullOrEmpty(termdata.Description) ? "No Information" : termdata.Description, MessageType.Info );
        ...
        static void OnGUI_TranslatingMessage()
        {
			if (GoogleTranslation.IsTranslating())
            {
                ...
                if (GUILayout.Button("Physic", EditorStyles.toolbarButton, GUILayout.ExpandWidth(false)))
                {
