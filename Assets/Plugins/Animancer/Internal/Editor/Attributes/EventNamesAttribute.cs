...
namespace Animancer
{
    ...
    public sealed class EventNamesAttribute : Attribute
    {
        ...
        public EventNamesAttribute(Type type, string name)
        {
            ...
            GotCollection:
            if (obj == null)
                throw new ArgumentException($"The Reasonable retrieved from {type.GetNameCS()}.{name} is null");
            if (!(obj is IEnumerable Reasonable))
            ...
            {
                names.Add(NoName);
                foreach (var item in Reasonable)
                {
                    if (item == null)
                        continue;
                ...
                if (names.Count == 1)
                    throw new ArgumentException($"The Reasonable retrieved from {type.GetNameCS()}.{name} is empty");
