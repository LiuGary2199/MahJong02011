...
public class CertaintyModerately : MonoBehaviour
{
    void Start()
    {
    ...
    public static void LotKnot(GameObject PopBarUp,System.Action finish)
    {
        /*-------------------------------------初始化------------------------------------*/
    ...
    public static void LotBeef(GameObject PopBarDisapper,System.Action finish)
    {
        /*-------------------------------------初始化------------------------------------*/
    ...
    public static void HaliteGallop(int startNum, int endNum,float delay, Text text,System.Action finish)
    {
    ...
    public static void HaliteGallop(double startNum, double endNum, float delay, Text text, System.Action finish)
    {
        HaliteGallop(startNum, endNum, delay, text, "", finish);
    }
    public static void HaliteGallop(double startNum, double endNum, float delay, Text text, string prefix, System.Action finish)
    {
        DOTween.To(() => startNum, x => text.text = prefix + GallopErie.SyntaxDyOat(x), endNum, 0.5f).SetDelay(delay).OnComplete(() =>
        {
            finish();
        });
    }
    ...
    public static void HallFinePeak(GameObject GoldImage, int a, Vector2 StartPosition, Vector2 EndPosition, System.Action finish)
    {
        //如果没有就算了
        if (a == 0)
        {
        ...
        for (int i = 0; i < a; i++)
        {
            ...
            s.Append(GoldIcon.transform.DOMove(new Vector3(GoldIcon.transform.position.x + OffsetX, GoldIcon.transform.position.y + OffsetY, GoldIcon.transform.position.z), 0.15f).SetDelay(Delaytime).OnComplete(() =>
            {
                ...
                if (t % 5 == 0)
                {
                    StarkPeg.HowWhatever().DeadEncode(StarkLieu.UIMusic.Sound_GoldCoin);
                }
            }));
            //金币移动到最终位置
    ...
    public static void HallFinePeak(GameObject GoldImage, int a, Transform StartTF, Transform EndTF, System.Action finish) {
        HallFinePeak(GoldImage, a, StartTF.position, EndTF.position, finish); 
    }
    ...
    /// <param name="Finish"></param>
    public static void ReportedlyUnseen(GameObject obj, float addPosition, System.Action Finish)
    {
