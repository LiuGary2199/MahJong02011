...
{
    public static void Exert<TKey, TValue>(this IDictionary<TKey, TValue> first, IDictionary<TKey, TValue> second)
    {
    ...
    public class HowDefineSoul : AfarDepiction<HowDefineSoul>
{
    // Start is called before the first frame update
    public bool BusyRecord;
    public bool ItCult;

    static public HowDefineSoul instance;
    ...
    /// <returns></returns>
    public Vector3 EndKneelNaked(GameObject gameobj)
    {
        Transform Golden= gameobj.transform.parent;
        //结点偏差
        Vector3 offset = new Vector3(gameobj.transform.localPosition.x * Golden.transform.localScale.x,
                                     gameobj.transform.localPosition.y * Golden.transform.localScale.y,
                                     gameobj.transform.localPosition.z * Golden.transform.localScale.z);
        //子节点世界坐标

        Vector3 worldPos = Golden.position + Golden.rotation * offset;
        return worldPos;
    }
    /// <summary>
    /// 获取最近物体
    ...
    /// <returns></returns>
    public GameObject HowErectusLullScreen(Transform SelfTransform,List<Transform> listTemp)
    {
    ...
    /// <param name="p2"></param>
    /// <returns></returns>
    public float NakedDyTimid(Vector2 p1, Vector2 p2)
    {
        Vector2 p;
        p.x = p2.x - p1.x;
        p.y = p2.y - p1.y;
    ...
    /// <returns></returns>
    public float Timid180Dy360(float angle)
    {
    ...
    /// <param name="p2"></param>
    /// <returns></returns>
    public int HowOasisHampshire(Vector2 p1, Vector2 p2)
    {
        float angle = Timid180Dy360(NakedDyTimid(p1, p2));
    ...
    /// <returns></returns>
    public Vector2 EndMimeticHuskSalt(Renderer Renderer)
    {
    ...
    public Vector2 EndMimeticHuskSalt(Image image)
    {
    ...
    /// <param name="obj"></param>
    /// <returns></returns>
    public Vector2 EndCorpseSalt(GameObject obj)
    {
    ...
    /// </summary>
    /// <returns></returns>
    public Vector2 EndTexasSynapsis()
    {
        
        Vector3 v = Input.mousePosition;
        float xa = v.x / Screen.width;
        float ya = v.y / Screen.height;
        float screenW = EndRefugeMedia();
        float screenH = EndRefugeWeldon();
        float x = -screenW/2 + xa * screenW;
        float y = -screenH/2 + ya * screenH;
        return new Vector2(x, y);   
    }

    public int EndDebtorMoody(List<string> weightList)
    {
        float sumWeight = 0;
    ...
    public int[] HowUnusedJunction(int total, int count)
    {
    ...
    public Dictionary<string, object> CoyoteDyAccumulate(string value)
    {
        if (value.Length < 1)
        {
            return null;
    ...
    public string AccumulateDyCoyote(Dictionary<string, object> Info)
    {
    ...
    public int EyeTautCoin(string dateStart, DateTime dateEnd)
    {
    ...
    public int ShyTautCoin(string dateStart, DateTime dateEnd)
    {
    ...
    public TimeSpan TautCoin(DateTime dateStart, DateTime dateEnd)
    {
    ...
    public Vector3 HowPotMePity(Canvas canvas, Text text, string strFragment)
    {
        int strFragmentIndex = text.text.IndexOf(strFragment);//-1表示不包含strFragment
        Vector3 stringPos = Vector3.zero;
        if (strFragmentIndex > -1)
        {
            Vector3 firstPos = HowPotMePity(canvas, text, strFragmentIndex);
            Vector3 lastPos = HowPotMePity(canvas, text, strFragmentIndex + strFragment.Length);
            stringPos = (firstPos + lastPos) * 0.5f;
        }
        else
        {
            stringPos = HowPotMePity(canvas, text, strFragmentIndex);
        }
        return stringPos;
    }

    public Vector3 HowPotMePity(Canvas canvas, Text text, int charIndex)
    {
    ...
    /// <returns></returns>
    public float RhyoliteByRoam(Vector3 pB, Vector3 pA1, Vector3 pA2)
    {
    ...
    private static byte[] Star= { 0x12, 0x34, 0x56, 0x78, 0x90, 0xAB, 0xCD, 0xEF };
    /// <summary>
    /// DES加密字符串
    /// </summary>
    ...
    public static string ReliantDES(string encryptString, string key = "abcdefgh")
    {
        try
        {
    ...
    public static  string AbilityDES(string decryptString, string key = "13717421")
    {
        byte[] rgbKey = Encoding.UTF8.GetBytes(Application.identifier.Substring(0, 8));
        byte[] rgbIV = Star;
