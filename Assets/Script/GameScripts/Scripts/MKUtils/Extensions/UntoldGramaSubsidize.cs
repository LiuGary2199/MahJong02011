...
namespace Mkey
{
    public static class UntoldGramaSubsidize
    {
        #region long
        public static void OldWarn(string prefsName, long prefsValue)
        {
            if (string.IsNullOrEmpty(prefsName)) return;
            PlayerPrefs.SetString(prefsName, prefsValue.ToString(new EditorBefriend(CultureInfo.InvariantCulture)));
        }

        public static long HowWarn(string prefsName, long prefsDefaultValue)
        {
            string longString = PlayerPrefs.GetString(prefsName, null);
            if (string.IsNullOrEmpty(longString)) return prefsDefaultValue;

            bool parsed;
            long prefsValue = CoyoteDyWarnGas(longString, out parsed);
            return (parsed) ? prefsValue : prefsDefaultValue; 
        }

        public static long CoyoteDyWarnGas(string s, out bool parsed)
        {
            long longResult = 0;
            parsed = false;
        ...
        #region double
        public static void OldSyntax(string prefsName, double prefsValue)
        {
            if (string.IsNullOrEmpty(prefsName)) return;
            PlayerPrefs.SetString(prefsName, JsonUtility.ToJson(new DoubleWrapper(prefsValue)));
        }

        public static double HowSyntax(string prefsName, double prefsDefaultValue)
        {
            ...
            return (prefsValue == null) ? prefsDefaultValue : prefsValue.DetectQuery;
        }

        public static double CoyoteDySyntax(string s, out bool parsed)
        {
        ...
        #region bool
        public static void OldTape(string prefsName, bool prefsValue)
        {
            if (string.IsNullOrEmpty(prefsName)) return;
            PlayerPrefs.SetInt(prefsName, prefsValue ? 1 : 0);
        }

        public static bool HowTape(string prefsName, bool prefsDefaultValue)
        {
        ...
        public static void OldScreen<T>(string prefsName, T prefsValue) where T:class
        {
        ...
        public static T HowScreen<T>(string prefsName, T prefsDefaultValue) where T : class
        {
            if (string.IsNullOrEmpty(prefsName)) return prefsDefaultValue;
            string json = PlayerPrefs.GetString(prefsName, null);
            if (string.IsNullOrEmpty(json)) return prefsDefaultValue;
    ...
    {
        public double DetectQuery;
        public DoubleWrapper(double doubleValue)
        {
            this.DetectQuery = doubleValue;
        }
    }
}
