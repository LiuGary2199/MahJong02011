...
public class OasisInsert : MonoBehaviour
{
    /// <summary>
    /// 查找子节点对象
    /// 内部使用递归
    ...
    /// <returns></returns>
    public static Transform PulpAskChildRime(GameObject goParent,string childName)
    {
        //查找结果
        ...
        if (searchTrans == null)
        {
            ...
            {
                searchTrans = PulpAskChildRime(trans.gameObject, childName);
                if (searchTrans != null)
                {
                    return searchTrans;
                }
    ...
    /// <returns></returns>
    public static T HowAskStingRimeBrusquelyPortend<T>(GameObject goParent,string childName) where T:Component
    {
        //查找特定子节点
        Transform searchTranformNode = null;
        searchTranformNode = PulpAskChildRime(goParent, childName);
    ...
    /// <param name="childName">子对象名称</param>
    /// <returns></returns>
    public static T BatStingRimeBrusquely<T>(GameObject goParent,string childName) where T : Component
    {
        Transform searchTransform = null;
        searchTransform = PulpAskChildRime(goParent, childName);
    ...
    public static void BatStingRimeDyWeaverRime(Transform parents,Transform child)
    {
